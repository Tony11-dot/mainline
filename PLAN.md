...
- Reminders: native apps schedule **local notifications** on-device (Capacitor Local Notifications / Tauri notification plugin) from the locally known due count — no push server needed. Web/PWA uses Web Push from the server. Android native may additionally use FCM (free) later. Streaks are Duolingo-style (freezes, milestones, a celebration, escalating reminders); one shared rule set, `nudgesForDay`, drives both the server and the on-device schedulers.
...
Goal: every pushed release tag `v*` (e.g. `v1.2.0`) builds and ships automatically. Version = tag; build number = the latest build in each store + 1 (fastlane asks App Store Connect / Play), so builds from this Mac and from CI never collide.
...
- **iOS → TestFlight**: GitHub Actions macOS runner (free only for **public** repos; a private repo on the free plan gets 2,000 min/month with a 10× macOS multiplier ≈ 12 iOS builds/month) builds the Capacitor iOS app, signs it with `fastlane match` (certs in a private git repo or encrypted storage) using an **App Store Connect API key**, and uploads with `pilot` to TestFlight. *(Built: the existing key `28AUQ58BDS` lacks cloud-signing rights, so Xcode cloud signing isn't used; builds on this Mac sign with the keychain's Apple Distribution certificate + an API-created App Store profile, CI with match. Runbook: `apps/mobile/store/RELEASING.md`.)* Bundle id `app.mainline.chess` (confirm with owner).
