...
- **fastlane** in `apps/mobile/fastlane`:
  - `ios build` / `ios beta` build a signed App Store `.ipa` and upload it to TestFlight.
  - `android build` / `android internal` build a signed `.aab` and upload it to the Play internal track.
  - `ios metadata` / `android metadata` push the store listing and screenshots.
  - `ios certificates` sets up CI signing with match.
  - Version comes from the tag. The build number is the store's latest + 1. "What's new" is the newest entry in this file.
- **First signed builds made on this Mac:** MainLine 1.0.0 (1).
  - iOS: `apps/mobile/build/ios/MainLine.ipa`. The bundle ID `app.mainline.chess` and an App Store profile were created through the existing API key.
  - Android: `app-release.aab`, signed with a new upload key in `secrets/`.
- **`.github/workflows/release.yml`**: tag `v*` → iOS on a macOS 26 runner to TestFlight, and Android to Play internal. The build artifacts are kept.
- **Store listing sources** in `apps/mobile/store`:
  - Texts for both stores.
  - Play icon and feature graphic, generated by `scripts/brand.mjs`.
  - 15 screenshots from the real app: iPhone 6.9″, iPad 13″, Android. Regenerate with `STORE_SHOTS=1 pnpm e2e store --project=desktop`.
- **`store/compliance.md`**: App Privacy label, Data safety form, content rating, target audience, review notes. **`store/RELEASING.md`**: the click-by-click runbook.
- **Privacy policy and terms** at `/privacy` and `/terms`, served by the API and linked from Settings. `#delete` is Play's account-deletion URL.
- **In-app deletion**: Settings → Your data → *Delete my account & data*. This deletes the server account, revokes the Lichess token, and wipes this device including push subscriptions. Guests can erase the device.
- **iOS privacy manifest** (`PrivacyInfo.xcprivacy`) matching the declared data.

**Fixed along the way**
- Portrait tablets (iPad) now stack the board above the panels instead of squeezing it next to them.
- Duplicate *White*/*Black* folders:
  - Cause: two concurrent library loads, or folders synced from another device.
  - Fix: loads are single-flight, and default roots merge into one deterministically, keeping every repertoire.

**Try it**: `cd apps/mobile && VERSION=1.0.0 BUILD_NUMBER=1 fastlane android build` (or `ios build`). Open `/privacy` on the dev server.
